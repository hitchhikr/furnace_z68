...
//  ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
  ...
  switch (romTarget) {
    ...
    case DIV_ROM_Z68: {
      bool z68ExportLoop = romConfig.getBool("loop", true);

      if (ImGui::Checkbox(_("loop"), &z68ExportLoop)) {
        altered = true;
      }
      if (altered) {
        romConfig.set("loop", z68ExportLoop);
      }
      break;
    }
    case DIV_ROM_ZSM: {
...
void FurnaceGUI::drawExport(bool Focus) {
  if (settings.exportOptionsLayout==1 || curExportType==GUI_EXPORT_NONE) {
    if (ImGui::BeginTabBar("ExportTypes")) {
      ...
      if (romExportExists) {
        if (ImGui::BeginTabItem(_("ROM"), 0, Focus == true ? ImGuiTabItemFlags_SetSelected : 0)) {
          drawExportROM(true);
          ImGui::EndTabItem();
        }
      }
