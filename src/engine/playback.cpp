    ...
    for (int j=0; j<curPat[i].effectCols; j++) {
      ...
      switch (effect) {
        ...
        case 0xfd: // virtual tempo num
          if (effectVal>0) 
          {
              virtualTempoN=effectVal;
          }
          break;
        case 0xfe: // virtual tempo den
          if (effectVal>0)
          {
              virtualTempoD=effectVal;
          }
          break;
...
*/
  double TempN;
  double TempD;
  TempN = (virtualTempoN * 2.0) / 5.0;
  TempD = (virtualTempoD * 2.0) / 5.0;
  if(TempN < 1.0) TempN = 1.0;
  if(TempD < 1.0) TempD = 1.0;
  NewTempo = divider * (TempN / TempD);
//  Temp = (virtualTempoD * 2.0) / 5.0;
 // NewTempo = divider + (divider - Temp);
